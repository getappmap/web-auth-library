...
function getCredentials(credentials) {
  return typeof credentials === "string" || credentials instanceof String ? Object.freeze(JSON.parse(credentials)) : Object.isFrozen(credentials) ? credentials : Object.freeze(credentials);
}
/**
...
function getPrivateKey(options) {
  ...
  return (0, _jose.importPKCS8)(credentials.private_key, "RS256");
}
/**
...
async function importPublicKey(options) {
  ...
  async function fetchKey() {
    ...
    if (!res.ok) {
      ...
      throw new _error.FetchError(error ?? "Failed to fetch the public key", {
        response: res
      });
    }
    ...
    if (!x509) {
      throw new _error.FetchError(`Public key "${keyId}" not found.`, {
        response: res
      });
    }
    const key = await (0, _jose.importX509)(x509, "RS256");
